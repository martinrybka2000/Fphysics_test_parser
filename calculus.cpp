...
Calculus::Calculus(Scalar const &node_variant, std::map<std::string, Scalar> const &node_map) : m_node_variant(node_variant), m_node_map(node_map)
{
}
...
Scalar Calculus::Partial(Scalar const &node_variant, Scalar const &with_respect_to_ptr)
{

    Scalar scalar = node_variant;

    // d/dx { x } = 1
    if (Node::Equivalent(scalar, with_respect_to_ptr))
    {
        return std::shared_ptr<NumberNode>(new NumberNode(1.0));
    }
    // d/dx { 1 } = 0
    else if (scalar->Type() == "NumberNode")
    {
        return std::shared_ptr<NumberNode>(new NumberNode(0.0));
    }
    // d/dx { y } = 0
    ...
    {
        return std::shared_ptr<AdditionNode>(new AdditionNode({Partial(scalar->Argument(0), with_respect_to_ptr),
                                                               Partial(scalar->Argument(1), with_respect_to_ptr)}));
    }
    // d/dx { f(x) - g(x) } = f'(x) - g'(x)
    else if (scalar->Type() == "SubtractionNode")
    {
        return std::shared_ptr<SubtractionNode>(new SubtractionNode({Partial(scalar->Argument(0), with_respect_to_ptr),
                                                                     Partial(scalar->Argument(1), with_respect_to_ptr)}));
    }
    // d/dx { f(x) * g(x) } = f'(x) * g(x) + g'(x) * f(x)
    else if (scalar->Type() == "MultiplicationNode")
    {
        return std::shared_ptr<AdditionNode>(new AdditionNode({std::shared_ptr<MultiplicationNode>(new MultiplicationNode({Partial(scalar->Argument(0), with_respect_to_ptr),
                                                                                                                           scalar->Argument(1)})),
                                                               std::shared_ptr<MultiplicationNode>(new MultiplicationNode({Partial(scalar->Argument(1), with_respect_to_ptr),
                                                                                                                           scalar->Argument(0)}))}));
    }
    // d/dx { f(x) / g(x) } = { f'(x) * g(x) - g'(x) * f(x) } / { g(x) }^2
    else if (scalar->Type() == "DivisionNode")
    {
        return std::shared_ptr<DivisionNode>(new DivisionNode({std::shared_ptr<SubtractionNode>(new SubtractionNode({std::shared_ptr<MultiplicationNode>(new MultiplicationNode({Partial(scalar->Argument(0), with_respect_to_ptr),
                                                                                                                                                                                 scalar->Argument(1)})),
                                                                                                                     std::shared_ptr<MultiplicationNode>(new MultiplicationNode({Partial(scalar->Argument(1), with_respect_to_ptr),
                                                                                                                                                                                 scalar->Argument(0)}))})),
                                                               std::shared_ptr<PowerNode>(new PowerNode({scalar->Argument(1),
                                                                                                         std::shared_ptr<NumberNode>(new NumberNode(2.0))}))}));
    }
    // d/dx { x^2 } = d/dx { e^{ ln(x) * 2 } } = e^{ ln(x) * 2 } * d/dx { ln(x) * 2 } = 2 * x^2 * x^{-1} = 2 * x
    else if (scalar->Type() == "PowerNode")
    {
        return Partial(std::shared_ptr<ExpNode>(new ExpNode({std::shared_ptr<MultiplicationNode>(new MultiplicationNode({std::shared_ptr<LnNode>(new LnNode({scalar->Argument(0)})),
                                                                                                                         scalar->Argument(1)}))})),
                       with_respect_to_ptr);
    }
    ...
    {
        return std::shared_ptr<MultiplicationNode>(new MultiplicationNode({scalar,
                                                                           Partial(scalar->Argument(0), with_respect_to_ptr)}));
    }
    // d/dx { ln { x^2 } } = { x^2 }^{-1} * d/dx { x^2 } = 2 * x * x^{-2} = 2 * x^{-1}
    else if (scalar->Type() == "LnNode")
    {
        return std::shared_ptr<MultiplicationNode>(new MultiplicationNode({std::shared_ptr<PowerNode>(new PowerNode({scalar->Argument(0),
                                                                                                                     std::shared_ptr<NumberNode>(new NumberNode(-1.0))})),
                                                                           Partial(scalar->Argument(0), with_respect_to_ptr)}));
    }
    else if (scalar->Type() == "SinNode")
    {
        return std::shared_ptr<MultiplicationNode>(new MultiplicationNode({std::shared_ptr<CosNode>(new CosNode({scalar->Argument(0)})),
                                                                           Partial(scalar->Argument(0), with_respect_to_ptr)}));
    }
    else if (scalar->Type() == "CosNode")
    {
        return std::shared_ptr<MultiplicationNode>(new MultiplicationNode({std::shared_ptr<MultiplicationNode>(new MultiplicationNode({std::shared_ptr<NumberNode>(new NumberNode(-1.0)),
                                                                                                                                       std::shared_ptr<SinNode>(new SinNode({scalar->Argument(0)}))})),
                                                                           Partial(scalar->Argument(0), with_respect_to_ptr)}));
    }
    else
    {
...
// Matrix Calculus::Gradient(Scalar const &node_variant, Scalar const &with_respect_to_11_ptr, Scalar const &with_respect_to_21_ptr, Scalar const &with_respect_to_31_ptr)
// {
...
// Scalar Calculus::Divergence(Scalar const &node_variant, Scalar const &with_respect_to_11_ptr, Scalar const &with_respect_to_21_ptr, Scalar const &with_respect_to_31_ptr)
// {
...
//     return std::shared_ptr<AdditionNode>(new AdditionNode({std::shared_ptr<AdditionNode>(new AdditionNode({Partial(matrix(0, 0), with_respect_to_11_ptr),
...
// Matrix Calculus::Curl(Scalar const &node_variant, Scalar const &with_respect_to_11_ptr, Scalar const &with_respect_to_21_ptr, Scalar const &with_respect_to_31_ptr)
// {
//     if (!std::holds_alternative<Matrix>(node_variant))
//     {
